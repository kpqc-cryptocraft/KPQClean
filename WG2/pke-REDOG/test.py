...
if 'Windows' == platform.system() :
    ...
    path = "/home/sage/DeskTop/REDOG_2022.11.21/reference_implementation/REDOG/REDOG.so"
...
if(a.value == 128):
    ...
    lambda_sp = 3
    t = 6 
    random_seed = [85784,32742,73760,25550,86491,18641,52489,99245,11715,41260,79960,67759,92355,94684,79953,63075,49755,72468,56930,86465,38631,76150,28254,46941,28316,22429,91050,42834,79804,88892]
    #random_seed = [85784,32742,73760,25550,86491,18641,52489,99245,11715,41260,79960,67759,92355,94684,79953]
    ran_len = len(random_seed)
    ...
    lambda_sp = 3
    t = 8
    random_seed = [28714, 89516,  42394,82755,50188,36117,26694,74624,55842,61380,91622,15632,45969,42827,69854]
    ran_len = len(random_seed)
elif(a.value == 256):
    n = 72
    k = 12
    ...
    lambda_sp = 3
    t = 10
    random_seed = [70964,38388,14433,23257,30998,55216,45581,28060,53574,71897,95668,51856,76274,83898,46265]
    ran_len = len(random_seed)
#print(n)
...
for j in range(0,1):
    ...
    for i in range(ran_len):
        ...
        if(a.value == 128):
            f = open('rsp_' + str(a.value) +'.rsp', 'a')
            f.write('\ncount = %d' % int(i_count))
            f.write('\nseed = %s' % (str(format(seed, 'x').upper())))
            f.write('\nmlen = %d' % l)
            f.write('\nmsg = %s' %(str(bin_to_hex(msg))))
            f.write('\npk = %s' %(str(bin_to_hex(pK[0]))))
            f.write('\n, %s' %(str(bin_to_hex(pK[1]))))
            f.write('\nsk = %s' %(str(bin_to_hex(sK[0]))))
            f.write('\n, %s' %(str(bin_to_hex(sK[1]))))
            f.write('\n, %s' %(str(bin_to_hex(sK[2]))))
            f.write('\n, %s' %(str(sK[3])))
            f.write('\nclen = %d' % (2*n-k))
            f.write('\nc = %s' %(str(bin_to_hex(enc_c))))
            f.write('\nd_msg = %s' %(str(bin_to_hex(dec_msg))))
            f.write('\n')
            f.close()
            print('write done')
            f = open('req_' + str(a.value) +'.req', 'a')
            f.write('\ncount = %d' % int(i_count))
            f.write('\nseed = %s' % (str(format(seed, 'x').upper())))
            f.write('\nmlen = %d' % l)
            f.write('\nmsg = %s' %(str(bin_to_hex(msg))))
