...
#include "sha2.h"
#include "fips202.h"
#include "rolloII_parameters.h"
#include "string.h"
#include "rbc_lrpc.h"
#include "rolloII_types.h"
#include "rolloII_parsing.h"
...
int rolloII_encaps(uint8_t* ct, uint8_t* ss, const uint8_t* pk) {
  ...
  random_source encSeedexpander, prng;
  random_init(&prng, RANDOM_SOURCE_PRNG);
  random_init(&encSeedexpander, RANDOM_SOURCE_SEEDEXP);
  ...
  //Computing m
  random_get_bytes(&prng, m, SHA512_BYTES);

  // Computing theta
  sha3_512(theta, m, SHA512_BYTES);

  //Seedexpander used to encrypt
  random_seed(&encSeedexpander, theta, theta + 32, SEEDEXPANDER_MAX_LENGTH);
  ...
  rbc_qre_set_random_pair_from_support(&encSeedexpander, E1, E2, E, ROLLOII_PARAM_R, 1);
...
int rolloII_decaps(uint8_t* ss, const uint8_t* ct, const uint8_t* sk) {
  ...
  dimE = rbc_lrpc_RSR(E, ROLLOII_PARAM_R, skTmp.F, ROLLOII_PARAM_D, xc->v, ROLLOII_PARAM_N);
  ...
  //Random error vectors
  rbc_qre_set_random_pair_from_support(&encSeedexpander, E1prime, E2prime, Eprime, ROLLOII_PARAM_R, 1);
  ...
  result = (rbc_qre_is_equal_to(ctTmp.syndrom, cprime.syndrom) == 1);
  result &= memcmp(ctTmp.d, cprime.d, SHA512_BYTES) == 0;
  result &= memcmp(ctTmp.v, cprime.v, SHA512_BYTES) == 0;
