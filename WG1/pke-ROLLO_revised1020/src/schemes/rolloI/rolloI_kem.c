...
#include "sha2.h"
#include "fips202.h"
#include "rolloI_parameters.h"
#include "string.h"
#include "rbc_lrpc.h"
#include "rolloI_types.h"
#include "rolloI_parsing.h"


int rolloI_keygen(uint8_t* pk, uint8_t* sk) {
...
int rolloI_encaps(uint8_t* ct, uint8_t* ss, const uint8_t* pk) {
  rolloI_publicKey pkTmp;
  ...
  //Support
  rbc_vspace_set_random_full_rank(&prng, E, ROLLOI_PARAM_R);
  ...
  rbc_qre_set_random_pair_from_support(&prng, E1, E2, E, ROLLOI_PARAM_R, 1);
...
int rolloI_decaps(uint8_t* ss, const uint8_t* ct, const uint8_t* sk) {
  rolloI_secretKey skTmp;
  rolloI_ciphertext ctTmp;
  ...
  dimE = rbc_lrpc_RSR(E, ROLLOI_PARAM_R, skTmp.F, ROLLOI_PARAM_D, xc->v, ROLLOI_PARAM_N);
