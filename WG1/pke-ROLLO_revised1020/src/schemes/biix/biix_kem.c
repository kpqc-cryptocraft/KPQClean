...
int biix_keygen(uint8_t* pk, uint8_t* sk) {
  ...
  uint8_t pkinv_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  uint8_t p_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
 
  uint8_t pk_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  uint8_t pk_POPI_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  uint8_t pk_PB_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  ...
  rbc_qre_set_arb_random(PNA, BIIX_PARAM_PNA_DEG, sk_PO_seed+16);
  rbc_qre_set_arb_random(PNB, BIIX_PARAM_PNA_DEG, sk_PO_seed+32);

  rbc_qre_from_string(pk_POPI,pk_POPI_st);
  rbc_qre_mul(PNA, PNA, P);
  rbc_qre_mul(PNB, PNB, P);

  
  rbc_qre_add(pk_POPI, pk_POPI, PNA);
  ...
  rbc_qre_mul(pk_PB, PNB, PO);
  rbc_qre_to_string(pk_PB_st,pk_PB);
  //Added



for(size_t i = 0 ; i < BIIX_PUBLIC_KEY_BYTES/3 ; ++i) 
  {
    pk[i]=pk_st[i];
    pk[i+BIIX_PUBLIC_KEY_BYTES/3]=pk_POPI_st[i];
    pk[i+2*BIIX_PUBLIC_KEY_BYTES/3]=pk_PB_st[i];
  }
...
int biix_sk_generate(uint8_t* sk_m, uint8_t* pk_xyinv1,  uint8_t* sk_PI_seed, uint8_t* sk_2PI) {
  ...
  uint8_t PI_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  ...
  uint8_t pk_st1[BIIX_PUBLIC_KEY_BYTES/3] = {0};
...
int biix_encaps(uint8_t* ct, uint8_t* ss, uint8_t* pk) {
  ...
  {
  	pk_st[i] = pk[i];
  	pk_POPI_st[i] = pk[i+BIIX_PUBLIC_KEY_BYTES/3];
    pk_PB_st[i] = pk[i+2*BIIX_PUBLIC_KEY_BYTES/3];
  }					
  
  biix_encaps_errgen(pk_st, pk_POPI_st,pk_PB_st, ct, ss);
  
  return 0;
}

int biix_encaps_errgen(uint8_t* pk1_st, uint8_t* pk_POPI_st,uint8_t* pk_PB_st, uint8_t* ct1_st, uint8_t* ss)
{
  ...
  rbc_qre_set_arb_random(PNC, BIIX_PARAM_PNC_DEG, PN_seed+16);
  ...
  rbc_qre_from_string(PB, pk_PB_st);
  ...
  rbc_qre_mul(PB, PNC,PB);
  rbc_qre_add(ct1Tmp.syndrom, ct1Tmp.syndrom, PB);
...
int biix_decaps(uint8_t* ss, const uint8_t* ct, uint8_t* sk) {
  ...
  unsigned char ctRes[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  ...
  unsigned char PO_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
  unsigned char PI_st[BIIX_PUBLIC_KEY_BYTES/3] = {0};
...
void rbc_qre_mod_sparse_from_string(rbc_qre o, const uint8_t* str) {
  ...
  rbc_poly unreduced;//2*modulus-1//Fixed as constants
  
  //BIIX_PARAM_ON+E_Byte2+1
  rbc_poly_init(&unreduced, BIIX_PARAM_ON-1);
  rbc_poly_set_zero(unreduced, BIIX_PARAM_ON-1);
  rbc_poly_from_string(unreduced, str);
  // Modular reduction
  for(int32_t k = 1 ; k < 3 ; k++)
  {
    for(int32_t i = unreduced->max_degree-modulus_degree  ; i >= (2-k) * modulus_degree ; i--) {
...
void rbc_qre_set_arb_random_pair_from_support(random_source* ctx, rbc_qre o1, rbc_qre o2, const rbc_vec support, uint8_t copy_flag) {
  //Predefined from
   uint32_t size =  BIIX_PARAM_ON-BIIX_PARAM_N-BIIX_PARAM_PNA_DEG-1;
